...
def discover_assets(scope: ScopeConfig, settings: ScoutSettings) -> tuple[list[Asset], list[str]]:
    ...
    for host in candidate_hosts:
        ...
        assets.append(host_asset(scope.program_id, host, ["scope_seed" if host in root_domains else "subfinder", "dnsx"], ip=ips[0] if ips else None))

    http_rows, http_logs = httpx.probe(candidate_hosts, httpx_path=settings.httpx_path, timeout=settings.timeout_seconds)
    ...
    for row in http_rows:
        ...
        paths = robots.discover(url, user_agent=settings.user_agent, timeout=settings.timeout_seconds)
        if paths:
        ...
        mcp_hits = mcp_detector.detect(url, user_agent=settings.user_agent, timeout=settings.timeout_seconds)
        if mcp_hits:
