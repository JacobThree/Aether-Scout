...
def detect(base_url: str, *, user_agent: str, timeout: int = 10, rate_limiter=None) -> list[dict]:
    found: list[dict] = []
    for path in MCP_CANDIDATE_PATHS:
        url = urljoin(base_url.rstrip("/") + "/", path.lstrip("/"))
        status, content_type = _head_or_get(url, user_agent=user_agent, timeout=timeout, rate_limiter=rate_limiter)
...
def _head_or_get(url: str, *, user_agent: str, timeout: int, rate_limiter=None) -> tuple[int | None, str | None]:
